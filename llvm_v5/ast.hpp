...
static std::map<std::string, std::map<std::string, Value *>> NamedValues;
...
static std::string currentFunc = "_emptyStruct";
static std::map<std::string, std::vector<Type *>> funcStructParam;
static  Value* funcStruct;
static std::map<std::string, Type*> funcStructType;
static std::map<std::string, std::string> parentFunc;
static int structCounter;
static Value* parentStruct; 
...
public:
  ...
  void llvm_compile_and_dump() {  
    ...
    findStructParam();
    funcStructType["_emptyStruct"] = StructType::create(TheContext, {}, "_emptyStruct");
    ...
    parentStruct = builder.CreateAlloca(funcStructType["_emptyStruct"]);
    builder.CreateCall(firstFunc, parentStruct);
...
public:
  ...
  virtual Value *compile() override {
    return builder.CreateLoad(NamedValuesTypes[currentFunc][str], NamedValues[currentFunc][str], str);
  }
...
public:
  ...
  virtual Value *getOffset(char* c, int levelOfRecursion) override{
    ...
    for(long unsigned int j=levelOfRecursion; j<ArrDimensions[currentFunc][c].size(); j++){
      sizeOfTheRestOftheArray *= ArrDimensions[currentFunc][c][j];
    }
  ...
  virtual Value *compile() override{
    ...
    if(strcmp(typeid(*lvalue).name(), "2Id")==0){
      if(isCharVar[currentFunc][id] == 1){
        return builder.CreateLoad(NamedValuesTypes[currentFunc][id], builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {expr->compile()}), id);
      }
      else{
        return builder.CreateLoad(NamedValuesTypes[currentFunc][id], builder.CreateGEP(Type::getInt32Ty(TheContext), NamedValues[currentFunc][id],  {expr->compile()}), id);
      }
      
    }
    else{
      Value *offset = getOffset(id, ArrDimensions[currentFunc][id].size());
      Value *elementPtr;
      if(isCharVar[currentFunc][id] == 1){
        elementPtr = builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {offset});
        return builder.CreateLoad(NamedValuesTypes[currentFunc][id],  elementPtr, id);
      }
      else{
        elementPtr = builder.CreateGEP(Type::getInt32Ty(TheContext), NamedValues[currentFunc][id],  {offset});
        return builder.CreateLoad(NamedValuesTypes[currentFunc][id],  elementPtr, id);
      }
    }
  }
  ...
  std::vector<Value *> compile_ExprList(bool libFunc = true, std::string funcName = ""){
    ...
    if(libFunc){
      ...
      for (Expr *c : expr_list) {      
        ...
        if(typeOfExpr == "Id"){
          if(isCharVar[currentFunc][id] == 1)
           v.push_back(builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {ConstantInt::get(Type::getInt32Ty(TheContext), 0)}));
          else
            v.push_back(builder.CreateGEP(Type::getInt32Ty(TheContext), NamedValues[currentFunc][id],  {ConstantInt::get(Type::getInt32Ty(TheContext), 0)}));
          continue;
        }
        ...
        if(isCharVar[currentFunc][id] == 1)
           v.push_back( builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {offset}));
          else
            v.push_back( builder.CreateGEP(Type::getInt32Ty(TheContext), NamedValues[currentFunc][id],  {offset}));
...
public:
  ...
  virtual Value *compile() override {
    ...
    if(expr != nullptr)
      builder.CreateStore(expr->compile(), NamedValues[currentFunc]["_returnValue"]);
...
public:
  ...
  virtual Value *compile() override {
    if(strcmp(typeid(*expr1).name(), "2Id")==0) //Den 3erw giati 8eleri 2Id ??
      builder.CreateStore(expr2->compile(), NamedValues[currentFunc][expr1->getName()]);
    else{
      char* id = expr1->getName();
      std::vector<int> dimensions = ArrDimensions[currentFunc][id];
  ...
  virtual Value *compile() override{
    if(strcmp(id,"writeInteger")==0){
      ...
      if (((expr_list->getExprList())[0])->getTypeOfExpr() == "String") {
        ...
        Value *s =  builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {ConstantInt::get(Type::getInt32Ty(TheContext), 0)});
        builder.CreateCall(writeString, std::vector<Value *> { s });
        return nullptr;
      }
      else{
        char* id = (expr_list->getExprList())[0]->getName();
        Value *offset = ((expr_list->getExprList())[0])->getOffset(id, ArrDimensions[currentFunc][id].size()-1);
        Value *s =  builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {offset});
        builder.CreateCall(writeString, std::vector<Value *> { s });
        return nullptr;
      }
    }
      ...
      if(((expr_list->getExprList())[1])->getTypeOfExpr() == "Id"){
        ...
        Value *n = ((expr_list->getExprList())[0])->compile();
        char* id = (expr_list->getExprList())[1]->getName();
        Value *offset = ((expr_list->getExprList())[1])->getOffset(id, ArrDimensions[currentFunc][id].size()-1);
        Value *s =  builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {offset});
        builder.CreateCall(readString, std::vector<Value *> { n, s });
        return nullptr;
      }
    ...
    if (((expr_list->getExprList())[0])->getTypeOfExpr() == "String") {
      ...
      char* id = (expr_list->getExprList())[0]->getName();
      s_1 =  builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {ConstantInt::get(Type::getInt32Ty(TheContext), 0)});
    }
    else{
      char* id = (expr_list->getExprList())[0]->getName();
      Value *offset = ((expr_list->getExprList())[0])->getOffset(id, ArrDimensions[currentFunc][id].size()-1);
      s_1 =  builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {offset});
    }
    ...
    if (((expr_list->getExprList())[1])->getTypeOfExpr() == "String") {
      ...
      char* id = (expr_list->getExprList())[1]->getName();
      s_2 =  builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {ConstantInt::get(Type::getInt32Ty(TheContext), 0)});
    }
    else{
      char* id = (expr_list->getExprList())[1]->getName();
      Value *offset = ((expr_list->getExprList())[1])->getOffset(id, ArrDimensions[currentFunc][id].size()-1);
      s_2 =  builder.CreateGEP(Type::getInt8Ty(TheContext), NamedValues[currentFunc][id],  {offset});
    }
...
/**************************************************************************************************/
/**************************************************************************************************/      
      Value* PassingStruct = funcStruct;
      ...
      if(currentFunc!=parentFunc[id]){
        ...
        if(currentFunc == id) 
        {
          PassingStruct = builder.CreateGEP(PointerType::getUnqual(funcStructType[parentFunc[currentFunc]]), PassingStruct,  {ConstantInt::get(Type::getInt32Ty(TheContext), 0)});
          ...
          while(parentFunc[tmpFuncName] != id){
            levelDropToParent++;
            tmpFuncName = parentFunc[tmpFuncName];
          }

          tmpFuncName = parentFunc[currentFunc];
          while(levelDropToParent > 0){
            PassingStruct = builder.CreateLoad(PointerType::getUnqual(funcStructType[tmpFuncName]), PassingStruct );
            PassingStruct = builder.CreateStructGEP(funcStructType[tmpFuncName], PassingStruct, 0);
            levelDropToParent--;
            tmpFuncName = parentFunc[tmpFuncName];
          }
          PassingStruct = builder.CreateLoad(PointerType::getUnqual(funcStructType[tmpFuncName]), PassingStruct );
      ...
      parameters.push_back(PassingStruct);
  ...
  virtual void findStructParam() override{
    ...
    for(int i = 0; i < IdListSize; i++){
      ...
      if(type == TYPE_int && !ref){
       ...
       funcStructParam[currentFunc].push_back(Type::getInt32PtrTy(TheContext));
      }
...
public:
  ...
  virtual Value *compile() override{
    ...
    if(!firstFuncDefined){
      parentFunc[currentFunc] = "_emptyStruct";
      funcStructParam[currentFunc].insert(funcStructParam[currentFunc].begin(), PointerType::getUnqual(funcStructType["_emptyStruct"]));
      ...
      FunctionType *funcType = FunctionType::get(Type::getVoidTy(TheContext), {PointerType::getUnqual(funcStructType["_emptyStruct"])}, false);
    ...
    if(parentFunc[currentFunc] != "_emptyStruct")
      funcStructParam[currentFunc].insert(funcStructParam[currentFunc].begin(), PointerType::getUnqual(funcStructType[parentFunc[currentFunc]]));
    funcStructType[currentFunc] = StructType::create(TheContext, funcStructParam[currentFunc], currentFunc+"_Struct");
    funcStruct = builder.CreateAlloca(funcStructType[currentFunc]);
    structCounter = 0;

    if(ReturnTypesOfFunc[id] == TYPE_int)
    {
      isCharVar[currentFunc]["_returnValue"] = 0;
      AllocaInst *Alloca  = builder.CreateAlloca(Type::getInt32Ty(TheContext), nullptr, "_returnValue");
      NamedValues[currentFunc]["_returnValue"] = Alloca;
      NamedValuesTypes[currentFunc]["_returnValue"] = i32;
    } 

    if(ReturnTypesOfFunc[id] == TYPE_char)
    {
      isCharVar[currentFunc]["_returnValue"] = 1;
      ...
      if(funcParamRefs.find(id)!= funcParamRefs.end()) {
        ...
        for(char *c: fparamsVector) {
          ...
          if(funcParamTypes[id][i] == i32){
            ...
            isCharVar[currentFunc][c] = 0;
            NamedValues[currentFunc][c] = builder.CreateStructGEP(funcStructType[currentFunc], funcStruct, ++structCounter, c);
            builder.CreateStore(std::next(FuncPtr[id]->arg_begin(), i), NamedValues[currentFunc][c]);
  ...
  std::vector<Func *> compile_LocalDef(){
    ...
    for(Func *ldef_elem : ldef_list){
     ...
     else if(ldef_elem->getTypeOfFunc() == "FuncDef")
      {
        ...
        funcParamTypes[id].push_back(PointerType::getUnqual(funcStructType[currentFunc]));
        if(ReturnTypesOfFunc[id] == TYPE_int) 
...
public:
  ...
  virtual Value *compile() override{
    ...
    if(head->getDataype() == TYPE_int)
      builder.CreateRet(builder.CreateLoad(NamedValuesTypes[currentFunc]["_returnValue"], NamedValues[currentFunc]["_returnValue"], "_returnValue"));
